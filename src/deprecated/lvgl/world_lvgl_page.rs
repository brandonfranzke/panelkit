//! World Page implementation with LVGL
//!
//! This module provides the "World" page for the PanelKit demo.
...
/// The World page implementation
pub struct WorldLvglPage {
    /// Base LVGL page
    base: LvglPage,
    
    /// Label for world text
    world_label: Option<Label>,
    
    /// Left arrow indicator
    left_indicator: Option<Arc>,
}
...
impl WorldLvglPage {
    ...
    pub fn new() -> Self {
        Self {
            base: LvglPage::new("World"),
            world_label: None,
            left_indicator: None,
        }
    }
    
    /// Create the world label
    fn create_world_label(&mut self) -> Result<()> {
    ...
    /// Create the left arrow indicator
    fn create_left_indicator(&mut self) -> Result<()> {
        let screen = self.base.get_screen();
        ...
        arc.set_pos(10, (self.base.height as i16) / 2 - 25);
        
        // Set arc properties
        arc.set_angles(0, 270); // Create a partial circle as an arrow
        arc.set_rotation(270); // Rotate to point left
...
impl Page for WorldLvglPage {
    fn init(&mut self) -> Result<()> {
        // Initialize the base page
        self.base.init()?;
        
        // Create UI elements
        self.create_world_label()?;
        self.create_left_indicator()?;
    ...
    fn handle_event(&mut self, event: &Event) -> Result<Option<String>> {
        ...
        match event {
            ...
            Event::Touch { x, y, action } => {
                // Check if touch is in the left side of the screen
                if *x < (self.base.width as i32) / 2 {
                    // Navigate to hello page on left side touch
                    return Ok(Some("hello".to_string()));
                }
            },
            _ => {}
    ...
    fn update_layout(&mut self, width: u32, height: u32) -> Result<()> {
        ...
        if let Some(label) = &self.world_label {
            label.set_align(Align::Center, 0, 0);
        }
        
        // Update indicator position (left side)
        if let Some(arc) = &self.left_indicator {
            arc.set_pos(10, (height as i16) / 2 - 25);
        }
